...
class SavesRepositoryImpl(
    ...
    override suspend fun getAllSaves(): List<Save> = withContext(Dispatchers.IO) {
        savesDao.getAll()
    }

    override suspend fun fetchCurrentSave(): Save? = withContext(Dispatchers.IO) {
        savesDao.loadCurrent()
    }

    override suspend fun loadFromId(id: Int): Save = withContext(Dispatchers.IO) {
